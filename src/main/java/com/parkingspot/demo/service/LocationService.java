...
import com.parkingspot.demo.error.NoDataFoundException;
	...
	public AddressResponse getResponse(String query) throws IOException, URISyntaxException {
		...
		if(root != null) {
			...
			if(array != null && !array.isEmpty()) {
				...
				throw new NoDataFoundException("No location found for given Input place");
			}
		...
		if(lattitude != null && longitude != null) {
			...
			List<Locations> items = new ArrayList<>();

				
			items.addAll(response1.getItems().stream().limit(3).collect(Collectors.toList()));
			items.addAll(response2.getItems().stream().limit(3).collect(Collectors.toList()));
			items.addAll(response3.getItems().stream().limit(3).collect(Collectors.toList()));

			if(items.isEmpty())
				throw new NoDataFoundException("No nearby place found");
			...
			throw new NoDataFoundException("Location information unavailable");
		}
		return response;
	}
} 
